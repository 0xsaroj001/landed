...
import { ExecutionLog, type LandedRecord, type StageEntry } from "./log.ts";
import { describePolicy, evaluateTransfer, normalizePolicy, type ExecutionPolicy, type NormalizedPolicy, type PolicyDecision } from "./policy.ts";
 ...
 */
export function keeperhub(options: KeeperHubExtensionOptions): KeeperHubExtension {
  const client =
    options.client ??
    new KeeperHubClient({
  ...
  const log = new ExecutionLog({ maxEntries: options.logMaxEntries, clock: options.clock });
  ...
  const runtime: KeeperHubRuntime = {
    ...
    async dryRun(intent) {
      ...
      const idempotencyKey = keyFor(reference, body);
      if (!decision.allowed) {
      ...
      try {
        ...
        if (error instanceof PreflightFailed) {
          return { reference, decision, simulation: error.simulation, idempotencyKey };
        }
    ...
    async transfer(intent) {
      ...
      try {
        const simulation = await client.simulateTransfer(body);
        log.push(record, "simulated", { gasEstimate: simulation.gasEstimate, from: simulation.from });
      } catch (error) {
        if (error instanceof PreflightFailed) {
          log.push(record, "preflight_failed", { code: error.code, failureKind: error.failureKind, revertReason: error.revertReason });
          log.finish(record, "preflight_failed", { code: "preflight_failed", message: error.message });
          throw new LandedError("preflight_failed", error.message, {
            reference,
            details: { code: error.code, failureKind: error.failureKind, revertReason: error.revertReason },
            cause: error,
          });
        }
      ...
      try {
        ...
        log.push(record, "accepted", { executionId: accepted.executionId, status: accepted.status, replayed: accepted.idempotentReplay });
        ...
        if (error instanceof ExecutionFailed) {
          ...
          log.push(record, "failed", { reason: error.reason, status: error.state.status, receipts: error.state.receipts, error: error.state.error });
          log.finish(record, "failed", { code: "execution_failed", message: error.message });
        ...
        if (error instanceof ExecutionUnconfirmed) {
          record.transactionHash = error.state.transactionHash;
          log.push(record, "unconfirmed", { status: error.state.status, waitedMs: error.waitedMs, transactionHash: error.state.transactionHash });
          log.finish(record, "unconfirmed", { code: "execution_unconfirmed", message: error.message });
          throw new LandedError("execution_unconfirmed", `${error.message}. Retry with the same reference, never a new one.`, {
            reference,
            details: {
              executionId: error.state.executionId,
              transactionHash: error.state.transactionHash,
    ...
    describe() {
      return {
        ...
        params: {
          ...
          idempotency: "sha256(reference|chainId|recipient|amount|token) shared with KeeperHub; same reference never pays twice",
          proof: ["executionId", "transactionHash", "transactionLink", "receipt.verified", "receipt.receiptStatus"],
          settlement: "payment finalizes only on receipt.verified === true && receiptStatus === 'success'",
        },
      };
    },
  };
  ...
  return {
    ...
    onManifestBuild(card: AgentManifest): AgentManifest {
      ...
      const existing = Array.isArray(capabilities.extensions) ? capabilities.extensions : [];
      const descriptor: Record<string, unknown> = { ...runtime.describe() };
      const extensions = [...existing.filter((e) => !(isRecord(e) && e.uri === descriptor.uri)), descriptor];
      return { ...card, capabilities: { ...capabilities, extensions } };
    },
  };
}

export function recordToJSON(record: LandedRecord): Record<string, unknown> {
  return { ...record };
}

function keyFor(reference: string, body: ReturnType<typeof canonicalTransferBody>): string {
...
function requireReference(reference: string): string {
  ...
  if (!trimmed) {
    throw new LandedError("invalid_request", "reference is required: the caller's stable id for this payment (invoice, task, period)");
  }
  return trimmed;
}
...
function wrapKeeperHubError(error: unknown, reference: string): LandedError {
  ...
  if (error instanceof KeeperHubError) {
    return new LandedError("keeperhub_error", error.message, {
      reference,
      details: { status: error.status, code: error.code, requestId: error.requestId, retryable: error.retryable },
      cause: error,
    });
  }
  return new LandedError("keeperhub_error", error instanceof Error ? error.message : String(error), { reference, cause: error });
}
