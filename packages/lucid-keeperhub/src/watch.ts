...
import { z } from "zod";
import { referenceSchema } from "./entrypoints.ts";
import { LandedError } from "./errors.ts";
import type { KeeperHubRuntime } from "./extension.ts";
import type { LandedRecord, StageEntry } from "./log.ts";
 ...
 */
export function keeperhubWatchEntrypoint(options: WatchEntrypointOptions = {}): EntrypointDef<typeof watchInputSchema, undefined> {
  const key = options.key ?? "watch";
  const timeoutMs = options.timeoutMs ?? 180_000;
  return {
    key,
    description: "Follow a reference live: each stage (policy, dry run, broadcast, receipt) as it happens, then the final record. Free.",
    ...
    stream: async (ctx, emit) => {
      ...
      if (!record) {
        await emit({ kind: "error", code: "not_found", message: `no execution for reference ${reference}`, retryable: false });
        return { status: "failed", error: { code: "not_found", message: `no execution for reference ${reference}` } };
      }
      ...
      await new Promise<void>((resolve) => {
        ...
        const timer = setTimeout(() => {
          enqueue({ kind: "control", control: "timeout", payload: { reference, timeoutMs } });
          finish();
        }, timeoutMs);
      ...
      return { output: summary(final), status: final.outcome ? "succeeded" : "cancelled" };
    },
  };
}

function stageEnvelope(entry: StageEntry): StreamPushEnvelope {
  return { kind: "control", control: "stage", payload: entry.detail ? { stage: entry.stage, at: entry.at, detail: entry.detail } : { stage: entry.stage, at: entry.at } };
}
...
function runtimeOf(runtime: unknown): KeeperHubRuntime {
  ...
  if (!slice) {
    throw new LandedError("not_installed", "keeperhub() extension is not installed on this agent");
  }
  return slice;
}
