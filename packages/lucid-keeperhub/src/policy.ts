import { canonicalAmount, canonicalChainId, normalizeAddress, type CanonicalTransferBody, type ChainInput } from "@landed/keeperhub-client";
...
export type PolicyDecision = { allowed: true } | { allowed: false; rule: PolicyRule; reason: string };
...
export function normalizePolicy(policy: ExecutionPolicy): NormalizedPolicy {
  ...
  const tokens = policy.tokens
    ? new Set(policy.tokens.map((t) => (t === "native" ? "native" : normalizeAddress(t))))
    : null;
  const recipients = policy.recipients ? new Set(policy.recipients.map(normalizeAddress)) : null;
  const maxAmount = policy.maxAmount !== undefined ? canonicalAmount(policy.maxAmount) : null;
...
export function evaluateTransfer(policy: NormalizedPolicy, body: CanonicalTransferBody): PolicyDecision {
  if (!policy.chains.has(body.chainId)) {
    return { allowed: false, rule: "chain", reason: `chain ${body.chainId} is not in the agent's policy (${[...policy.chains].join(", ")})` };
  }
  const token = body.tokenAddress ?? "native";
  if (policy.tokens && !policy.tokens.has(token)) {
    return { allowed: false, rule: "token", reason: `token ${token} is not in the agent's policy` };
  }
  if (policy.recipients && !policy.recipients.has(body.recipientAddress)) {
    return { allowed: false, rule: "recipient", reason: `recipient ${body.recipientAddress} is not in the agent's allowlist` };
  }
  if (policy.maxAmount !== null && compareDecimal(body.amount, policy.maxAmount) > 0) {
    return { allowed: false, rule: "max_amount", reason: `amount ${body.amount} exceeds the agent's maximum of ${policy.maxAmount}` };
  }
  return { allowed: true };
}

/** A serialisable summary for the agent card. Never includes secrets. */
export function describePolicy(policy: NormalizedPolicy): Record<string, unknown> {
  return {
