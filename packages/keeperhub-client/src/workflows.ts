import { canonicalAmount, canonicalChainId, normalizeAddress } from "./idempotency.ts";
import type { ChainInput, WorkflowDefinition } from "./types.ts";
 ...
 */
export function scheduledTransferWorkflow(input: ScheduledTransferWorkflowInput): WorkflowDefinition {
  ...
  const tokenAddress = input.tokenAddress ? normalizeAddress(input.tokenAddress) : undefined;

  const triggerConfig: Record<string, unknown> = { triggerType: "Schedule", scheduleCron: cron };
  if (input.timezone) {
  ...
  const definition: WorkflowDefinition = {
    name: input.name,
    nodes: [
      { id: "trigger", type: "trigger", data: { label: "Schedule", config: triggerConfig } },
      { id: "payout", type: "action", data: { label: tokenAddress ? "Transfer token" : "Transfer native", config: actionConfig } },
    ],
