...
export class KeeperHubClient {
  ...
  constructor(options: KeeperHubClientOptions) {
    ...
    if (options.apiKey.startsWith("wfb_")) {
      throw new Error("KeeperHubClient: wfb_ keys authenticate webhooks only; use an organization kh_ key");
    }
  ...
  async me(): Promise<{ id?: string; email?: string; walletAddress: string | null; raw: Record<string, unknown> }> {
    ...
    const me: { id?: string; email?: string; walletAddress: string | null; raw: Record<string, unknown> } = {
  ...
  async listChains(): Promise<unknown[]> {
    ...
    return Array.isArray(json) ? json : ((json as { chains?: unknown[] }).chains ?? []);
  }
  ...
  executeTransfer(req: TransferRequest, options: ExecuteOptions): Promise<ExecutionAccepted> {
    return this.execute(TRANSFER_PATH, canonicalTransferBody(req), options);
  }

  executeContractCall(req: ContractCallRequest, options: ExecuteOptions): Promise<ExecutionAccepted> {
    return this.execute(CONTRACT_CALL_PATH, canonicalContractCallBody(req), options);
  }
  ...
  async getStatus(executionId: string): Promise<ExecutionState> {
    ...
    for (;;) {
      ...
      if (attempt < this.maxAttempts && (error instanceof RateLimited || res.status >= 500)) {
        await this.backoff(error instanceof RateLimited ? error.retryAfterSeconds * 1000 : 500 * attempt, "status retry");
        continue;
      }
      throw error;
    }
  ...
  async waitForTerminal(executionId: string, options: WaitOptions = {}): Promise<ExecutionState> {
    ...
    let state = options.initial && options.initial.executionId === executionId ? options.initial : undefined;
    if (state?.terminal) {
      return state;
    }
    for (;;) {
      if (state) {
        const hintMs = state.pollIntervalHint === null ? 2000 : state.pollIntervalHint * 1000;
  ...
  async transferAndVerify(req: TransferRequest, options: VerifyOptions = {}): Promise<VerifiedExecution> {
    ...
    const accepted = await this.execute(TRANSFER_PATH, body, { idempotencyKey });
    return this.verify(accepted, options);
  }

  async contractCallAndVerify(req: ContractCallRequest, options: VerifyOptions = {}): Promise<VerifiedExecution> {
    ...
    const accepted = await this.execute(CONTRACT_CALL_PATH, body, { idempotencyKey });
    return this.verify(accepted, options);
  }

  /** Turn an accepted broadcast into proof, or throw with the exact reason it is not proof. */
  async verify(accepted: ExecutionAccepted, options: WaitOptions = {}): Promise<VerifiedExecution> {
  ...
  private async simulate(path: string, body: object): Promise<SimulationResult> {
    const res = await this.request("POST", path, { body: { ...body, simulate: true } });
    const json = await this.json(res);
    const record = asRecord(json);
  ...
  private async execute(path: string, body: object, options: ExecuteOptions): Promise<ExecutionAccepted> {
    if (!options.idempotencyKey || !options.idempotencyKey.trim()) {
      throw new Error("KeeperHubClient: idempotencyKey is required for a broadcast");
    }
    ...
    for (;;) {
      ...
      let res: Response;
      try {
        res = await this.request("POST", path, { body, idempotencyKey: key, attempt });
      } catch (networkError) {
        transientFailures += 1;
      ...
      if (error instanceof RateLimited || res.status >= 500) {
        ...
        const waitMs = error instanceof RateLimited ? error.retryAfterSeconds * 1000 : 500 * transientFailures;
  ...
  private async request(
    ...
    this.emit(
      options.idempotencyKey
        ? { type: "request", method, path, attempt, idempotencyKey: options.idempotencyKey }
    ...
    this.emit(
      requestId
        ? { type: "response", method, path, status: res.status, ms: this.now() - started, requestId }
        : { type: "response", method, path, status: res.status, ms: this.now() - started },
    );
    return res;
  }
  ...
  private mapError(res: Response, json: unknown): KeeperHubError {
      ...
      typeof record.detail === "string"
        ...
        : typeof record.error === "string"
          ...
          : typeof record.message === "string"
            ? record.message
            : `HTTP ${res.status}`;
    const requestId = res.headers.get("x-request-id") ?? (typeof record.request_id === "string" ? record.request_id : undefined);
    ...
    if (res.status === 409 && code === "idempotency_conflict") {
      const original = typeof record.originalExecutionId === "string" ? record.originalExecutionId : null;
    ...
    return new KeeperHubError(`KeeperHub ${res.status}: ${detail}`, { ...base, retryable: res.status >= 500 });
  }
...
function requireTaskId(options: VerifyOptions): string {
  if (!options.taskId || !options.taskId.trim()) {
    throw new Error("KeeperHubClient: pass idempotencyKey or a stable taskId so a retry cannot pay twice");
  }
...
function parseSimulation(record: Record<string, unknown>): SimulationResult {
  ...
  for (const field of ["status", "from", "to", "value", "gasEstimate", "code", "failureKind", "revertReason"] as const) {
...
function parseAccepted(json: unknown): ExecutionAccepted {
  ...
  if (typeof record.executionId !== "string") {
    throw new KeeperHubError("KeeperHub accepted the request without an executionId", { status: 202, body: json });
  }
...
export function parseState(json: unknown, res?: Pick<Response, "headers">): ExecutionState {
  const record = asRecord(json);
  const hintHeader = res?.headers.get("x-poll-interval-hint") ?? null;
  const hint = hintHeader === null || hintHeader === "" ? null : Number(hintHeader);
  const pollIntervalHint = hint === null || !Number.isFinite(hint) ? null : hint;
  const status = typeof record.status === "string" ? record.status : "unknown";
  const receipts = Array.isArray(record.receipts) ? record.receipts.map(parseReceipt) : [];
  const state: ExecutionState = {
    ...
    receipts,
    pollIntervalHint,
    terminal: pollIntervalHint === null ? TERMINAL_STATUSES.has(status) : pollIntervalHint === 0,
    raw: record,
  };
  for (const field of ["type", "network", "transactionHash", "transactionLink", "gasUsedWei", "gasPriceWei", "createdAt"] as const) {
    const v = record[field];
    if (typeof v === "string") {
      state[field] = v;
    }
...
export function assertVerified(state: ExecutionState, replayed = false): VerifiedExecution {
  ...
  const receipt = state.receipts.find((r) => r.hash === state.transactionHash) ?? state.receipts[0]!;
