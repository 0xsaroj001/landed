...
import type { ExecutionStatus, FetchLike, ReceiptStatus } from "./types.ts";
...
export function createMockKeeperHub(options: MockOptions = {}) {
  ...
  const json = (status: number, body: unknown, headers: Record<string, string> = {}): Response =>
    new Response(JSON.stringify(body), {
      status,
      headers: { "content-type": "application/json", "x-request-id": `req_${++counter}`, ...headers },
    });
  ...
  const fetchImpl: FetchLike = async (input, init) => {
    ...
    if (headers.authorization !== `Bearer ${apiKey}`) {
      return json(401, { error: "unauthorized", detail: "Missing or invalid authentication" });
    }
    ...
    if (method === "GET" && statusMatch) {
      ...
      const status = script[Math.min(execution.polls, script.length - 1)] ?? execution.status;
      execution.polls += 1;
      ...
      if (terminal) {
        ...
        hintHeaders["x-poll-interval-hint"] = String(options.nonTerminalHint ?? 2);
      }
      const receipts =
          ...
          ? [
              {
                ...
                receiptStatus: options.receiptStatus ?? (status === "failed" ? "reverted" : "success"),
      ...
      return json(
        200,
        {
          ...
          status,
          type: execution.path.endsWith("transfer") ? "transfer" : "contract-call",
          network: "84532",
          transactionHash: execution.hash,
          transactionLink: execution.hash ? `https://sepolia.basescan.org/tx/${execution.hash}` : undefined,
          ...
          result: null,
          error: status === "failed" ? "Contract call failed: Error(revert)" : null,
    ...
    if (method === "POST" && (url.pathname === "/api/execute/transfer" || url.pathname === "/api/execute/contract-call")) {
      const record = (body ?? {}) as Record<string, unknown>;
      if ("simulate" in record) {
        if (record.simulate !== true) {
          return json(400, { error: "invalid_input", detail: "simulate must be the JSON boolean true" });
        }
      ...
      const bodyHash = createHash("sha256").update(JSON.stringify(sortKeys(record))).digest("hex");
      ...
      if (key) {
        ...
        if (existing) {
          ...
          if (existing.execution) {
            return json(202, { ...acceptedBody(existing.execution), idempotentReplay: true });
          }
        }
      }
      if (serverErrorsLeft > 0) {
        serverErrorsLeft -= 1;
        return json(500, { error: "internal_error", detail: "upstream hiccup" });
      }
      if (rateLimitLeft > 0) {
        rateLimitLeft -= 1;
        return json(429, { error: "rate_limited", detail: "Too many requests" }, { "retry-after": "1" });
      }
      if (key && (options.inProgressAttempts ?? 0) > 0 && !idempotency.has(scope)) {
        idempotency.set(scope, { bodyHash, execution: undefined, inProgressLeft: (options.inProgressAttempts ?? 0) - 1 });
      ...
      const execution: StoredExecution = {
        ...
        hash: options.withoutHash ? undefined : `0x${(executions.size + 1).toString(16).padStart(64, "a")}`,
        polls: 0,
      ...
      if (key) {
        const existing = idempotency.get(scope);
        idempotency.set(scope, { bodyHash, execution, inProgressLeft: existing?.inProgressLeft ?? 0 });
      }
...
function acceptedBody(execution: StoredExecution): Record<string, unknown> {
  const body: Record<string, unknown> = { executionId: execution.id, status: execution.status };
