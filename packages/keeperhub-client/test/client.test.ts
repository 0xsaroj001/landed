...
import type { ClientEvent } from "../src/types.ts";
import { createMockKeeperHub, virtualClock, type MockOptions } from "./mock-keeperhub.ts";

const USDC = "0x036CbD53842c5426634e7929541eC2318f3dCF7e";
const RECIPIENT = "0x742d35Cc6634C0532925a3b844Bc454e4438f44e";
const transfer = { chainId: "base-sepolia", recipientAddress: RECIPIENT, amount: "0.01", tokenAddress: USDC };

function setup(mockOptions: MockOptions = {}, clientOptions: { apiKey?: string; maxAttempts?: number; inProgressMaxWaitMs?: number } = {}) {
...
describe("construction", () => {
  ...
  it("strips trailing slashes from the base URL", () => {
    expect(new KeeperHubClient({ apiKey: "kh_x", baseUrl: "https://a.b///" }).baseUrl).toBe("https://a.b");
  });
});
...
describe("auth and probing", () => {
  ...
  it("maps 401 to AuthError", async () => {
    ...
    await expect(client.simulateTransfer(transfer)).rejects.toBeInstanceOf(AuthError);
  });
...
describe("simulate", () => {
  ...
  it("turns a 400 wouldRevert into PreflightFailed with the attributed code", async () => {
    const { client, mock } = setup({ simulate: () => ({ wouldRevert: true, code: "insufficient_balance", failureKind: "validation" }) });
    const error = await client.simulateTransfer(transfer).catch((e: unknown) => e);
  ...
  it("treats an EVM revert as preflight failure too", async () => {
    const { client } = setup({ simulate: () => ({ wouldRevert: true, failureKind: "revert", revertReason: "ERC20: transfer amount exceeds balance" }) });
    const error = (await client.simulateTransfer(transfer).catch((e: unknown) => e)) as PreflightFailed;
    expect(error).toBeInstanceOf(PreflightFailed);
...
describe("execute", () => {
  it("requires an idempotency key", async () => {
    const { client } = setup();
    await expect(client.executeTransfer(transfer, { idempotencyKey: "" })).rejects.toThrow(/idempotencyKey/);
  });

  it("broadcasts with the Idempotency-Key header and returns the execution id", async () => {
    const { client, mock } = setup();
    const accepted = await client.executeTransfer(transfer, { idempotencyKey: "abc" });
    ...
    expect(mock.calls.at(-1)!.headers["idempotency-key"]).toBe("abc");
  });

  it("replays instead of paying twice when the same key and body are re-sent", async () => {
    const { client, mock } = setup();
    const first = await client.executeTransfer(transfer, { idempotencyKey: "abc" });
    const second = await client.executeTransfer({ ...transfer, amount: "0.010", chainId: 84532 }, { idempotencyKey: "abc" });
  ...
  it("surfaces a conflict with the original execution id and never rotates the key", async () => {
    ...
    const error = (await client.executeTransfer({ ...transfer, amount: "0.02" }, { idempotencyKey: "abc" }).catch((e: unknown) => e)) as IdempotencyConflict;
  ...
  it("waits through 409 in_progress under the same key", async () => {
    ...
    const accepted = await client.executeTransfer(transfer, { idempotencyKey: "abc" });
    expect(accepted.executionId).toBe("direct_1");
    const keys = new Set(broadcasts(mock).map((c) => c.headers["idempotency-key"]));
  ...
  it("gives up on in_progress after the budget without rotating", async () => {
    const { client, mock } = setup({ inProgressAttempts: 50 }, { inProgressMaxWaitMs: 3000 });
    await expect(client.executeTransfer(transfer, { idempotencyKey: "abc" })).rejects.toBeInstanceOf(IdempotencyInProgress);
  ...
  it("retries a 5xx under the same key and stops at maxAttempts", async () => {
    ...
    const error = (await exhausted.client.executeTransfer(transfer, { idempotencyKey: "abc" }).catch((e: unknown) => e)) as KeeperHubError;
  ...
  it("retries a thrown network error under the same key", async () => {
    ...
    const clock = virtualClock();
    const client = new KeeperHubClient({ apiKey: "kh_test", fetch: flaky, sleep: clock.sleep, now: clock.now });
    const accepted = await client.executeTransfer(transfer, { idempotencyKey: "abc" });
    expect(accepted.executionId).toBe("direct_1");
    expect(mock.executions.size).toBe(1);
  });
...
describe("status and waiting", () => {
  it("parses receipts, sponsorship and the poll hint", async () => {
    ...
    expect(state.receipts[0]).toMatchObject({ verified: true, receiptStatus: "success", blockNumber: 31_000_000 });
  });

  it("keeps polling through unconfirmed using the hint as the interval", async () => {
    const { client, clock } = setup({ executeStatus: "unconfirmed", statusScript: ["unconfirmed", "unconfirmed", "completed"], nonTerminalHint: 3 });
    const accepted = await client.executeTransfer(transfer, { idempotencyKey: "abc" });
    const state = await client.waitForTerminal(accepted.executionId, { maxWaitMs: 60_000 });
    expect(state.status).toBe("completed");
    expect(clock.sleeps).toEqual([3000, 3000]);
  });

  it("decides terminality from the hint, not the status string", async () => {
    const { client } = setup({ executeStatus: "settling", statusScript: ["settling", "completed"], nonTerminalHint: 1 });
    const accepted = await client.executeTransfer(transfer, { idempotencyKey: "abc" });
  ...
  it("clamps the interval between min and max", async () => {
    const { client, clock } = setup({ executeStatus: "unconfirmed", statusScript: ["unconfirmed", "completed"], nonTerminalHint: 600 });
    const accepted = await client.executeTransfer(transfer, { idempotencyKey: "abc" });
  ...
  it("throws ExecutionUnconfirmed after the budget and never re-sends", async () => {
    const { client, mock } = setup({ executeStatus: "unconfirmed", statusScript: ["unconfirmed"], nonTerminalHint: 2 });
    const accepted = await client.executeTransfer(transfer, { idempotencyKey: "abc" });
    const error = (await client.waitForTerminal(accepted.executionId, { maxWaitMs: 7000 }).catch((e: unknown) => e)) as ExecutionUnconfirmed;
...
describe("transferAndVerify (the whole safe sequence)", () => {
  it("simulates, broadcasts under a derived key, waits, and returns proof", async () => {
    ...
    const proof = await client.transferAndVerify(transfer, { taskId: "invoice-1" });
    ...
    const { simulate: _s, ...simulatedBody } = posts[0]!.body as Record<string, unknown>;
    expect(posts[1]!.body).toEqual(simulatedBody);
    expect(posts[1]!.headers["idempotency-key"]).toMatch(/^[0-9a-f]{64}$/);
    expect(events.map((e) => e.type)).toEqual(
      expect.arrayContaining(["request", "response", "simulated", "accepted", "status", "verified"]),
    );
  });
  ...
  it("does not broadcast when the dry run fails", async () => {
    const { client, mock } = setup({ simulate: () => ({ wouldRevert: true, code: "insufficient_balance" }) });
    await expect(client.transferAndVerify(transfer, { taskId: "t" })).rejects.toBeInstanceOf(PreflightFailed);
    expect(broadcasts(mock)).toHaveLength(0);
    expect(mock.executions.size).toBe(0);
  });

  it("refuses to call a reverted receipt landed", async () => {
    const { client } = setup({ executeStatus: "failed", receiptStatus: "reverted" });
    const error = (await client.transferAndVerify(transfer, { taskId: "t" }).catch((e: unknown) => e)) as ExecutionFailed;
  ...
  it("reports a replay when the same work is verified twice", async () => {
    const { client, mock } = setup();
    const first = await client.transferAndVerify(transfer, { taskId: "invoice-9" });
    const second = await client.transferAndVerify(transfer, { taskId: "invoice-9" });
    expect(second.executionId).toBe(first.executionId);
  ...
  it("verifies contract calls with the same rules", async () => {
    ...
    const proof = await client.contractCallAndVerify(
      { chainId: 84532, contractAddress: USDC, functionName: "transfer", functionArgs: `["${RECIPIENT}", "10000"]` },
      { taskId: "call-1" },
    );
    expect(proof.transactionHash).toMatch(/^0x/);
    expect(mock.calls.filter((c) => c.path === "/api/execute/contract-call")).toHaveLength(2);
  });
});
